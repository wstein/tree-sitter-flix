...
- `queries/highlights.scm` is checked in CI by `ts_query_ls check -f queries/`, which is a
  **formatter** check, not just a validity check — it fails on layout, so run
  `ts_query_ls format queries/` after editing (the binary is not vendored; fetch it from the
  ts_query_ls releases page). Node-type validity is caught separately by `tree-sitter build`.
