...
- `queries/` holds five files. `highlights`, `injections`, `locals` and `tags` are declared in
  `tree-sitter.json` and consumed by the CLI and editors; `indents.scm` and `folds.scm` are not
  in the schema and are read directly by Neovim. `injections.scm` sends `regex"..."` to the
  regex grammar and `///` doc comments to markdown, so both need those parsers installed to show
  any effect. Exercise a query without an editor via
  `tree-sitter query queries/locals.scm FILE.flix`.
- `tree-sitter.json` lists `src/scanner.c` under `external-files`; without it a scanner-only
  edit can be served from a stale shared object.
- All of `queries/` is checked in CI by `ts_query_ls check -f queries/`, which validates
